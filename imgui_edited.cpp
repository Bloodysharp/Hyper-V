...
#include <string>
 ...
 /*   namespace font
    ...
    bool KeybindRojas(const char* label, const char* description, int* key)
    {
        ImGuiWindow* window = ImGui::GetCurrentWindow();
        if (window->SkipItems) return false;
        ...
        static std::map<ImGuiID, key_state1> anim;
        auto it_anim = anim.find(id);

        if (it_anim == anim.end())
        {
            anim.insert({ id, key_state1() });
            it_anim = anim.find(id);
        }

        it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? c::element::text_active : hovered ? c::element::text_hov : c::element::text, ImGui::GetIO().DeltaTime * 6.f);

        window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(c::element::background), c::element::roundingoxy);

        RenderTextColor(font::lexend_regular, rect.Min + ImVec2(10, 0), rect.Max, GetColorU32(c::element::text), description, ImVec2(0.0, 0.8));


        /*    GetWindowDrawList()->AddRectFilled(rect.Min + ImVec2(width - 80, 0), rect.Max, GetColorU32(c::element::background), c::element::rounding);
            GetWindowDrawList()->AddRectFilledMultiColor(rect.Min, rect.Max - ImVec2(80, 0), GetColorU32(c::element::background, 0.f), GetColorU32(c::element::background, 1.f), GetColorU32(c::element::background, 1.f), GetColorU32(c::element::background, 0.f), c::element::rounding);*/

        window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(c::element::background_widget), c::element::roundingoxy);

        RenderTextColor(font::lexend_bold, rect.Min + ImVec2(10, 0), rect.Max, GetColorU32(c::element::text_active), label, ImVec2(0.0, 0.2));

        RenderTextColor(font::lexend_bold, clickable.Min, clickable.Max, GetColorU32(c::element::text_active), buf_display, ImVec2(0.5f, 0.5f));
    ...
    bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags)
    {
        ...
        RenderTextColor(font::inter_bold, parent_window->DC.CursorPos - ImVec2(-12, 35), parent_window->DC.CursorPos + ImVec2(size_arg.x, 0), GetColorU32(c::accent), name, ImVec2(0.0, 0.5f));
    ...
    bool CheckboxWithPopup(bool popup_box, const char* label, const char* hint, bool* v, int* key, int* mode, bool* value, bool keybind, bool picker, float color[4], ImGuiColorEditFlags flags)
    {
        ...
        state.text = ImLerp(state.text, *v ? c::accent : hovered ? c::element::text_hov : c::element::text, g.IO.DeltaTime * 12.f);
    ...
    bool edited::Checkbox(const char* label, bool* v)
    {
        ...
        const ImVec2 pos = window->DC.CursorPos;
        const ImVec2 check_size(18, 16); // Tamaño del checkbox

        const ImRect total_bb(pos, pos + ImVec2(check_size.x + label_size.x + style.FramePadding.x * 2, check_size.y));
        ...
        it_anim->second.circle_radius = ImLerp(it_anim->second.circle_radius, *v ? 10.f : 0.f, g.IO.DeltaTime * 6.f);
        ImColor color = ImColor(c::accent);

        window->DrawList->AddRect(total_bb.Min + ImVec2(0, 0), total_bb.Min + ImVec2(check_size.x, check_size.y), color, 3.f);
        ...
        PushClipRect(total_bb.Min + ImVec2(1, 1), total_bb.Min + ImVec2(check_size.x - 1, check_size.y - 1), true);
        window->DrawList->AddCircleFilled(total_bb.Min + ImVec2(check_size.x / 2, check_size.y / 2), it_anim->second.circle_radius, ImColor(c::accent.x, c::accent.y, c::accent.z, it_anim->second.circle_alpha));
        PopClipRect();

        if (*v)
            RenderCheckMark(window->DrawList, total_bb.Min + ImVec2(check_size.x / 2 - it_anim->second.checkmark_size / 2, check_size.y / 2 - it_anim->second.checkmark_size / 2), ImColor(255, 255, 255), it_anim->second.checkmark_size);

        window->DrawList->AddText(total_bb.Min + ImVec2(check_size.x + style.FramePadding.x + 10, (total_bb.Max.y - total_bb.Min.y) / 2 - CalcTextSize(label).y / 2), ImColor(255, 255, 255, 75), label);
    ...
    struct button_stateRojas
    {
        ImVec4 background, text, button;
    };

    bool edited::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
    {
        ImGuiWindow* window = GetCurrentWindow();
        ...
        static std::map<ImGuiID, button_stateRojas> anim;
        auto it_anim = anim.find(id);

        if (it_anim == anim.end())
        {
            anim.insert({ id, button_stateRojas() });
            it_anim = anim.find(id);
        }

        it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::element::background : c::element::background, g.IO.DeltaTime * 6.f);
        it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::accent : c::element::text, g.IO.DeltaTime * 6.f);

        ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), c::element::rounding);
        ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(c::accent), c::element::rounding);
    ...
    bool edited::Button(const char* label, const ImVec2& size_arg)
    {
    ...
    bool BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
    {
        ...
        state.text = ImLerp(state.text, state.opened_combo ? c::element::text_active : hovered ? c::accent : c::element::text, g.IO.DeltaTime * 6.f);
        ...
        RenderTextColor(font::inter_medium_widget, rect.Min + ImVec2(10, 0), rect.Max, GetColorU32(c::accent), preview_value, ImVec2(0.0, 0.49f));
    ...
    void esp_preview(ImTextureID player_preview, bool* nickname, float nick_color[4], bool* weapon, float weapon_color[4], int* hp, float hp_color[4], bool* zoom, float zoom_color[4], bool* bomb, float bomb_color[4], bool* c4, float c4_color[4], bool* money, float money_color[4], bool* hit, float hit_color[4], bool* esp_box, float box_color[4], bool* hp_line, float hp_line_color[4])
    {
        ...
        const ImRect player(pos + ImVec2((width - 250) / 2, 60), pos + ImVec2((width + 255) / 2, 345));
        const ImRect box(pos + ImVec2((width - 500) / 2, 40), pos + ImVec2((width + 180) / 2, 347));
        ...
        GetWindowDrawList()->AddImage(player_preview, player.Min, player.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::accent));

        static std::vector<std::string> partyNames = { "     Person Target", "                ", "               </> Developer:Rojas", "        ", "Advanced Npc"/*, "Enemy", "Run", "Gun"*/ };

        PushFont(font::inter_medium_small);
        PushStyleColor(ImGuiCol_Text, GetColorU32(c::accent));
